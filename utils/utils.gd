...
static func get_evolution_options(creature: CreatureStats) -> Array[Card]:
	var options := CardPile.new()

	var rare_ids := creature.move_ids.filter(func(id: String) -> bool:
		var data = MoveData.moves.get(id)
		return data and data.get("rarity", "common") in ["rare", "uncommon"]
	)

	if rare_ids.is_empty():
		rare_ids = creature.move_ids
